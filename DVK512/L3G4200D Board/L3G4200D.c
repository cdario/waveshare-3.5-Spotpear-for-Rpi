...
void write_buffer8(u8 adr, u8 *data,uint8_t f)
{
	...
	bcm2835_spi_transfer(adr|0x40);//地址 地址自动加1
  for(i=0;i<f;i++)
...
u8 read_buffer(u8 adr)
{
	...
	bcm2835_spi_transfer(adr|0x80);//1000 0000读取地址
...
void read_buffer8(u8 adr, u8 *data,uint8_t f)
{
	...
	bcm2835_spi_transfer(adr|0xc0);//1100 0000读取地址自动加1
  for(i=0;i<f;i++)
